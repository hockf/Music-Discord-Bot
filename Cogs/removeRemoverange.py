...
class CogRemoveRemoverange(commands.Cog):
    ...
    #         return await ctx.channel.send(f"{ctx.author.mention} Indexs have to be numbers!")
    #     if (int(index2) -1) > len(self.bot.music[ctx.guild.id]["musics"]):
    #         return await ctx.channel.send(f"{ctx.author.mention} Indexs are invalid!")
    #     if index1 >= index2:
    #         return await ctx.channel.send(f"{ctx.author.mention} The second index have to be bigger than the fisrt index invalid!")
