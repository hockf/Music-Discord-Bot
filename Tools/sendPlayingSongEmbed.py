...
async def sendPlayingSongEmbed(self, channel, track): 
    ...
    embed.add_field(name="Loop :", value=isLoop.replace("1", f"Yes").replace("0", f"No"), inline=True)
    embed.add_field(name="Loop queue :", value=isLoopQueue.replace("1", f"Yes").replace("0", f"No"), inline=True)
